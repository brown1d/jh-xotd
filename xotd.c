...
int main(int argc, char *argv[])
{
  ...
  setsockopt(sock, SOL_SOCKET, SO_REUSEADDR /*|SO_DEBUG */ , &on,
	     sizeof on);
...
clear:
  if (isVerbose)
    printd("TCP connection closed (tap inbound), lci=%d.",
	   xot->lci);
...
int write_xot(struct xot *xot, const unsigned char *buf, int len)
{
  ...
  if (writen(xot->sock, (unsigned char *)&head, sizeof head) !=
      sizeof head)
    return 0;
